...
$.fn.simplePagination = function(options){
	...
	return this.each(function(){
		...
		function refresh_page_navigation(page_number){
			...
			$('#' + self + ' .simplePagination-navigation').html(first_html + previous_html + page_numbers_html + next_html + last_html);
		}

		function refresh_current_page(page_number){
			$('#' + self + ' .simplePagination-current-page-text').text('Page ' + page_number + ' of ' + page_count);
		}

		function refresh_current_items(page_number, page_range_min, page_range_max){
			$('#' + self + ' .simplePagination-current-items-text').text('Showing ' + page_range_min + '-' + page_range_max + ' of ' + item_count);
		}
		...
		function refresh_page_information(page_number){
			var page_range_min = page_number * settings.items_per_page - settings.items_per_page,
				page_range_max = page_range_min + settings.items_per_page;
		...
		$('#' + self + ' .simplePagination-navigation').on('click', 'a', function(e){
		...
		$('#' + self + ' .simplePagination-update-items-per-page').change(function(){
			settings.items_per_page = +$(this).val();//force Number type with + OR parseInt()
			page_count = Math.ceil(item_count / settings.items_per_page);
			refresh_page_information(1);
		});
	});
